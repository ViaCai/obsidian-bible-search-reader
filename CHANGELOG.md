...
### 新增
- **内置数据源支持**：插件现在支持自带圣经数据，安装后开箱即用，无需额外下载或配置。
- **数据源切换**：设置中可选择「使用内置数据」或「使用外置数据」，随时切换。
- **首次运行引导**：首次启用插件时自动弹出引导窗口，帮助用户选择数据源模式并自动下载。
- **一键下载内置数据**：内置数据模式下，提供「下载 bible-data.json」按钮，自动从 GitHub Release 下载到插件目录。
- **一键下载外置文档**：外置数据模式下，提供「下载并解压」按钮，自动从 GitHub Release 下载 ZIP 并解压到 Vault，同时自动配置路径。
- **下载进度显示**：下载时实时显示文件大小和下载速度。
- **自动检测更新**：设置中开启后，启动时自动检测 GitHub 新版本并在右上角弹出提示。
- **手动检查更新**：设置中提供「检查更新」按钮，可手动触发版本检测。
- **自动更新安装**：检测到新版本后，可一键下载并安装 main.js + manifest.json + styles.css。
- **更新日志弹窗**：更新成功后自动弹出窗口显示版本更新内容。
- **数据生成脚本**：提供 `build-bible-data.js` 脚本，插件作者可将圣经文档编译成 `bible-data.json` 随插件发布。

### 改进
- **设置界面重构**：新增数据源选择区域，内置/外置数据的不同选项动态显示，并附带模式说明。
- **内置数据状态显示**：设置中实时显示内置数据的卷数、条数等信息。
- **兼容旧版本**：自动检测旧版双目录配置，默认切换到外置数据模式，避免升级后无法使用。
- **跨平台下载解压**：桌面端支持 Windows（PowerShell）和 macOS/Linux（unzip）自动解压。
- **禁用插件清理**：禁用插件时自动关闭所有圣经检索视图和投影窗口，避免残留。

### 修复
- **内置数据读取**：修复 `this.manifest.dir` 不可靠导致的内置数据检测失败问题，增加 Vault Adapter 和多路径 fallback。
- **搜索结果显示异常**：修复 `highlightKeywords` 中未定义变量 `patterns` 导致的渲染中断。
