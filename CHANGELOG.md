...
- **全局/本页操作区分**：阅读视图新增全局操作区（全局全选/取消/复制/投影）与本页操作区（本页全选/复制/复制经文/复制纲目），逻辑清晰分离。
...
- **主题直接显示**：阅读视图中书卷主题直接显示在内容开头，不再占用额外标题栏。
- **章节标题独立显示**：阅读视图章节标题与操作按钮分行排列，避免布局拥挤。
...
- **主题选中刷新**：修复阅读视图中主题卡片选中状态无法正确刷新的问题。
